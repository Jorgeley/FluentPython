...
async def fake_long_task(limit: int = 1000):
    ...
    for i in range(limit):
        if i != 0 and i % 1000000 == 0:
            print(
                f"sleeping {asyncio.tasks.current_task().get_name()} at {i}"
            )
            # this makes the longer task to give the others a chance to run,
            # like in parallel
...
async def main():
    ...
    # if we need the results, we have to wait for them, otherwise all goes
    # to background
